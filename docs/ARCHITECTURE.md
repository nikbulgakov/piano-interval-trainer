...
Web Audio также изолируется от доменной логики. Звук реагирует на нормализованные MIDI Note On/Off события и пользовательские настройки, но не влияет на генерацию заданий, проверку ответа или счётчики. Тембр «Пианино» использует только локальные MP3-сэмплы из `public/audio/piano/salamander`, остальные тембры строятся oscillator-based.
...
Экран настроек хранит точку возврата: `home`, `interval-setup` или `note-setup`. Router не нужен, потому что URL-маршруты и прямые ссылки на внутренние экраны не являются требованием. MIDI adapter и обе конфигурации тренировки принадлежат `App`, поэтому переход на главный экран не сбрасывает подключение и выбранные параметры. Экран настроек приложения использует тот же общий MIDI-блок, что и настройки тренировок, поэтому подключение и выбранный вход едины для всех экранов.
...
- запросить разрешение после действия пользователя или при best-effort восстановлении ранее сохранённого входа;
...
Если ранее был сохранён выбранный MIDI-вход, hook может сделать best-effort автозапрос MIDI-доступа при загрузке приложения. Без сохранённого входа доступ остаётся ручным, чтобы главный экран не открывал permission prompt без контекста.
