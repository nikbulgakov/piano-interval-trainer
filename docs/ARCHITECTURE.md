...
Web Audio также изолируется от доменной логики. Звук реагирует на нормализованные MIDI Note On/Off события и пользовательские настройки, но не влияет на генерацию заданий, проверку ответа или счётчики. Тембр «Пианино» использует локальные MP3-сэмплы из `public/audio/piano/salamander`, остальные тембры строятся oscillator-based.
...
│   ├── NoteTrainingSetup.tsx
│   ├── NoteStartPanel.tsx
│   ├── NoteSetupScreen.tsx
│   ├── NotePracticeScreen.tsx
│   ├── TrainingSetup.tsx
│   ├── StartPanel.tsx
│   ├── SetupScreen.tsx
│   ├── SettingsScreen.tsx
...
├── domain/
│   ├── music.ts
│   ├── noteTrainingConfig.ts
│   ├── noteTasks.ts
│   ├── noteSequentialSession.ts
│   ├── tasks.ts
│   ├── trainingConfig.ts
...
```text
                 ┌─► interval-setup ─► interval-practice ─► results
home ────────────┤
  │              └─► note-setup ─────► note-practice ─────► results
  └─► settings          │
                        └─► settings
```

Экран настроек хранит точку возврата: `home`, `interval-setup` или `note-setup`. Router не нужен, потому что URL-маршруты и прямые ссылки на внутренние экраны не являются требованием. MIDI adapter и обе конфигурации тренировки принадлежат `App`, поэтому переход на главный экран не сбрасывает подключение и выбранные параметры.
...
Последовательная сессия одиночных нот использует те же фазы ожидания отпускания, но принимает одну клавишу. После ошибки новая попытка разрешается только после полного отпускания, а после правильного ответа отпускание вытягивает следующую ноту из отдельного shuffle bag.
